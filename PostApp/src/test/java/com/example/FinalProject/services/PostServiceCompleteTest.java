...
public class PostServiceCompleteTest {
    ...
    void addComment_shouldAddCommentAndSavePost() {
        ...
        // When
        Post result = postService.addComment("post123", newComment);

        // Then
        verify(postRepository).findById("post123");
        verify(postRepository).save(testPost);
        assertEquals(2, result.getComments().size());
        assertEquals(newComment, result.getComments().get(1));
    }
    ...
    void getComments_shouldReturnCommentsList() {
        ...
        // When
        List<Comment> result = postService.getComments("post123");
    ...
    void updateComment_shouldUpdateCommentAtGivenIndex() {
        ...
        // When
        Post result = postService.updateComment("post123", 0, updatedComment);

        // Then
        verify(postRepository).findById("post123");
        verify(postRepository).save(testPost);
        assertEquals(1, result.getComments().size());
        assertEquals("Updated comment", result.getComments().get(0).getContent());
        assertEquals(5, result.getComments().get(0).getLikes());
        assertEquals(2, result.getComments().get(0).getDislikes());
    }
    ...
    void updateComment_shouldThrowExceptionForInvalidIndex() {
        ...
        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            postService.updateComment("post123", 5, updatedComment);
        });

        assertTrue(exception.getMessage().contains("Invalid comment index"));
        verify(postRepository, never()).save(any(Post.class));
    }

    @Test
    void deleteComment_shouldRemoveCommentAtGivenIndex() {
        // Given
        when(postRepository.findById("post123")).thenReturn(Optional.of(testPost));
        when(postRepository.save(any(Post.class))).thenReturn(testPost);

        // When
        Post result = postService.deleteComment("post123", 0);
    ...
    @Test
    void deleteComment_shouldThrowExceptionForInvalidIndex() {
        // Given
        when(postRepository.findById("post123")).thenReturn(Optional.of(testPost));

        // When & Then
        Exception exception = assertThrows(IllegalArgumentException.class, () -> {
            postService.deleteComment("post123", 5);
        });
